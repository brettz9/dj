...
DJ.createAndReturn = function createAndReturn (options) {
    var method,
        Constructor = options.constructs || DJ.createConstructor(options.inherits, options.constructor);
    for (method in options.methods) {
        Constructor.prototype[method] = (function (method) {
            return function () {
                options.methods.super = Constructor.prototype[method];
                options.methods[method].apply(options.methods, arguments);
            };
        }(method));
    }
    return Constructor;
...
DJ.createAndExport = function createAndExport (options) {
    ...
    DJ.exportClass(Constructor, options.name);
    return Constructor;
};
...
DJ.exportClass = function exportClass (clss, name) {
    name = name || clss.name;
    if (define !== undef && define.amd) { // AMD might not allow us to do this dynamically
        ...
        var mod = module !== undef ? module.exports : window;
        mod[name] = clss;
    }
};
